...
class LocationCell: UITableViewCell
{
    ...
    func InitializeUI()
    {
        ...
        ColorSwatch = UIView(frame: CGRect(x: 220, y: 5, width: 100, height: LocationCell.CellHeight - 10))
    ...
    func SetData(Name: String, Location: GeoPoint2, Color: UIColor, TableWidth: CGFloat)
    {
        ...
        let NewX = TableWidth - (ColorSwatch!.frame.width + 95)
        let NewFrame = CGRect(x: NewX, y: ColorSwatch!.frame.origin.y, width: ColorSwatch!.frame.size.width, height: ColorSwatch!.frame.size.height)
